<head>
<title> Persos </title>
<?php include ('../../Outils/header.php'); ?>
<meta charset='UTF-8'>
</head>

<br><br><br><br>
<body>
  <div class="w3-row-padding w3-grayscale">
    <div class="w3-col l3 m6 w3-margin-bottom">
	  <img src="/img/WC_2_Young.png" alt="Wesley" style="width:100%"><br />
	  <img src="/img/W_C_Young_injured.png" alt="Wesley" style="width:100%"><br />
	  <img src="/img/Wesley_with_scars_2.png" alt="Wesley" style="width:100%"><br />
	  <img src="/img/Wesley_Grand_Healed.png" alt="Wesley" style="width:100%"><br />
      <img src="/img/Wesley_Grand_23.png" alt="Wesley" style="width:100%"><br />
	  <img src="/img/Wesley.png" alt="Wesley" style="width:100%">
      <h3>Nom défini par le joueur</h3>
      <p class="w3-opacity">Chevalier-Assassin</p>
      <p>Il est le digne prince du Pays de Norowa.</p>
      <p><button class="w3-button w3-light-grey w3-block">En savoir plus</button></p>
    </div>
    <div class="w3-col l3 m6 w3-margin-bottom">
      <img src="/img/Raigel.png" alt="Raigel" style="width:100%">
	  <img src="/img/A_Raigel_faceset_1.png" alt="Raigel" style="width:100%">
	  <img src="/img/A_Raigel_transformation.png" alt="Raigel" style="width:100%">
	  <img src="/img/A_Raigel_1.png" alt="Raigel" style="width:100%">
      <h3>Raigel</h3>
      <p class="w3-opacity">Chevalier D'Alcyone</p>
      <p>...</p>
      <p><button class="w3-button w3-light-grey w3-block">En savoir plus</button></p>
    </div>
    <div class="w3-col l3 m6 w3-margin-bottom">
      <img src="/img/Firyia_true.png" alt="Raigel" style="width:100%">
	  <img src="/img/Firyia_true_char.png" alt="Raigel" style="width:100%">
      <h3>Nom défini par le joueur</h3>
      <p class="w3-opacity">???</p>
      <p>Une aventurière qui a pour but de découvrir le monde et apporter la paix à ce monde.</p>
      <p><button class="w3-button w3-light-grey w3-block">En savoir plus</button></p>
    </div>
    <div class="w3-col l3 m6 w3-margin-bottom">
      <img src="/w3images/team4.jpg" alt="Dan" style="width:100%">
      <h3>Dan Star</h3>
      <p class="w3-opacity">Architect</p>
      <p>Phasellus eget enim eu lectus faucibus vestibulum. Suspendisse sodales pellentesque elementum.</p>
      <p><button class="w3-button w3-light-grey w3-block">Contact</button></p>
    </div>
  </div>
</body>
  
